    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(timeFrame))
        {
            ...
            if (string.Compare(queryValues["Selection_Clicked"], "2") == 0)
            {
                ...
                g = ma.GetPlanData(timeFrameVals, isMonth, dataYear, planLevelTableName, dataField, keys, true, productID, queryValues);

                ma.ProcessGrid(detailedGrid, isMonth, g, timeFrameVals, queryValues);

                if (getPageCount)
                    g = ma.GetPlanData(timeFrameVals, isMonth, dataYear, planLevelTableName, dataField, keys, true, productID, n);
            }           
        }
